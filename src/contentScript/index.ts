...
function renderTabList(openedTabs: any[], closedTabs: any[] = []) {
  ...
  if (openedTabs.length > 0) {
    html += `<li class="tab-group-header">Opened Tabs</li>`;
    openedTabs.forEach((t) => {
      const faviconUrl = t.favIconUrl || '';
      const faviconHtml = faviconUrl
        ? `<img src="${faviconUrl}" alt="" class="tab-favicon" onerror="this.style.display='none'; this.nextElementSibling.style.display='inline-block';" />
           <span class="tab-favicon-default" style="display:none;">${defaultTabIconSvg}</span>`
        : `<span class="tab-favicon-default">${defaultTabIconSvg}</span>`;
      html += `<li data-tab-id="${t.id}" data-tab-type="opened" class="result-item ${itemIndex === selectedIndex ? "selected" : ""}">
        ${faviconHtml}
  ...
  if (closedTabs.length > 0) {
    html += `<li class="tab-group-header">Recently Closed Tabs</li>`;
    closedTabs.forEach((t) => {
      ...
      const faviconHtml = faviconUrl
        ? `<img src="${faviconUrl}" alt="" class="tab-favicon" onerror="this.style.display='none'; this.nextElementSibling.style.display='inline-block';" />
           <span class="tab-favicon-default" style="display:none;">${defaultTabIconSvg}</span>`
        : `<span class="tab-favicon-default">${defaultTabIconSvg}</span>`;
      html += `<li data-tab-url="${t.url}" data-tab-type="closed" class="result-item ${itemIndex === selectedIndex ? "selected" : ""}">
        ${faviconHtml}
        <span class="tab-title">${t.title}</span>
      </li>`;
      itemIndex++;
